...
class Cell:
    ...
    def __init__(self, t=30000, L=0, N=200, trans_speed=200, k_on=0, k_off=0,
             avalanche_on=True, thresh=1, num_release=5,
             L_mod=True, build_size=1, decay_size=1, D=1, ava_power=2.85,
        ...
        self.trans_speed = 2*t_step*trans_speed #2um, from Alex Chien and Ahmet Yildiz
        self.rms_disp = (2*D*1.75*self.t_step)**.5 #um
        self.decay_sizeMS = .01*decay_size #m/s
        self.decay = .01 * decay_size #m/s
        self.decay_size = .01 * t_step * decay_size #meters in one time step
        self.build_size = .00125 * build_size
    ...
    def avalanche(self):
        ...
        if num_in_base > self.thresh:
            distr = int((num_in_base-self.thresh+10) * np.random.weibull(1) + 1)
